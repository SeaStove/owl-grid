...
function App() {
  ...
  const [square, setSquare] = useState([0, 1, 2, 3, 4, 5, 6, 7, 8]);
  const [rows, setRows] = useState<TeamsObject>();
  const [cols, setCols] = useState<TeamsObject>();
  const [accessToken, setAccessToken] = useState();
  const [owlData, setOwlData] = useState<OwlData>();
  ...
  useEffect(() => {
    async function getOwlData() {
      const res = await axios.get("https://us.api.blizzard.com/owl/v1/owl2", {
  ...
  useEffect(() => {
    if (owlData && owlData?.teams) {
      ...
      const colsSelectedTeams: TeamsObject = shuffledTeamIds
        .slice(0, 3)
        .reduce((result: { [key: string]: TeamData }, teamId) => {
          result[teamId] = teamsData[teamId];
          return result;
        }, {});
      setCols(colsSelectedTeams);

      // Pick the next 3 teams for state 2
      const rowsSelectedTeams: TeamsObject = shuffledTeamIds
        .slice(3, 6)
        .reduce((result: { [key: string]: TeamData }, teamId) => {
          result[teamId] = teamsData[teamId];
          return result;
